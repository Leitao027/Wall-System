...
arc = Tunnel.getInterface("system_wall")
...
RegisterCommand("am",function()
    ...
    if wall then
		TriggerEvent("Notify","verde","Modo admin ON.",5000)
		arc.reportwallLog("WALL ON")
	else
		TriggerEvent("Notify","vermelho","Modo admin OFF.",5000)
...
RegisterCommand("lines",function()
	...
	if lines then
		TriggerEvent("Notify","verde","Linhas admin ON.",5000)
		arc.reportlinesLog("LINHAS ON")
	else
		TriggerEvent("Notify","vermelho","Linhas admin OFF.",5000)
					...
					if ((NetworkIsPlayerActive( id )) and GetPlayerPed(id) ~= PlayerPedId()) then	
						if admin and (PlayerPedId() ~= GetPlayerPed(id)) then
							if GetPlayerPed(id) ~= -1 and players[id] ~= nil then
								...
								if distance <= 500 and not IsEntityVisible(GetPlayerPed(id)) then
